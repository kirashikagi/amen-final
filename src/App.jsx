...
import { List, X, Check, Disc, Plus, Image as ImageIcon } from 'lucide-react'; 
...
const THEMES = {
  dawn: { 
    ...
    text: 'text-stone-800', subText: 'text-stone-500', 
    overlay: 'bg-[#78350f]/5', 
    button: 'border border-stone-800/10 hover:bg-stone-800/5', 
  ...
  morning: { 
    ...
    cardBg: 'bg-white/60 backdrop-blur-3xl shadow-sm border border-white/50', 
    text: 'text-slate-800', subText: 'text-slate-500', 
    overlay: 'bg-sky-900/5', 
    button: 'border border-slate-800/10 hover:bg-slate-800/5', 
    activeButton: 'bg-sky-900 text-white shadow-lg',
    menuBg: 'bg-white/90 backdrop-blur-3xl text-slate-800 border-l border-white/20'
  },
  day: { 
    id: 'day', label: 'День', bgImage: '/day.jpg', 
    fallbackColor: '#fdfce7', 
    cardBg: 'bg-[#fffff0]/60 backdrop-blur-3xl shadow-sm border border-white/40', 
    text: 'text-stone-900', subText: 'text-stone-600', 
    overlay: 'bg-yellow-900/5', 
    button: 'border border-stone-900/10 hover:bg-stone-900/5', 
    activeButton: 'bg-amber-900 text-white shadow-lg',
    menuBg: 'bg-[#fffff0]/90 backdrop-blur-3xl text-stone-900 border-l border-white/20'
  },
  sunset: { 
    id: 'sunset', label: 'Закат', bgImage: '/sunset.jpg', 
    fallbackColor: '#fff1f2', 
    cardBg: 'bg-[#fff1f2]/60 backdrop-blur-3xl shadow-sm border border-white/40', 
    text: 'text-rose-950', subText: 'text-rose-700', 
    overlay: 'bg-rose-900/10', 
    button: 'border border-rose-900/10 hover:bg-rose-900/5', 
    activeButton: 'bg-rose-900 text-white shadow-lg',
    menuBg: 'bg-[#fff1f2]/90 backdrop-blur-3xl text-rose-950 border-l border-white/20'
  },
  evening: { 
    id: 'evening', label: 'Вечер', bgImage: '/evening.jpg', 
    fallbackColor: '#f5f3ff', 
    cardBg: 'bg-[#2e1065]/40 backdrop-blur-3xl shadow-sm border border-white/10', 
    text: 'text-white', subText: 'text-purple-200', 
    overlay: 'bg-[#2e1065]/40', 
    button: 'border border-white/20 hover:bg-white/10', 
    activeButton: 'bg-white text-purple-900 shadow-lg',
  ...
  midnight: { 
    ...
    cardBg: 'bg-black/40 backdrop-blur-3xl shadow-sm border border-white/10', 
    ...
    menuBg: 'bg-black/80 backdrop-blur-3xl text-slate-200 border-l border-white/10'
  }
};
                    ...
                    {publicPosts.map(post => {
                         ...
                         return (
                             <Card key={post.id} theme={theme} className="!p-5">
                                 ...
                                 <button onClick={() => toggleLike(post.id, post.likes)} className={`w-full py-3 text-[10px] font-medium uppercase tracking-widest transition rounded-xl ${liked ? theme.activeButton : 'bg-current bg-opacity-5 hover:bg-opacity-10'}`}>
                ...
                <motion.div key="profile" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="text-center py-10">
                    <Card theme={theme} className="py-12">
                        ...
                        <div className="grid grid-cols-3 gap-3 mb-10">
                           {Object.values(THEMES).map(t => (
                             <button 
                               ...
                               className={`h-12 rounded-xl relative overflow-hidden transition-all duration-300 ${currentThemeId === t.id ? 'ring-2 ring-offset-2 ring-current scale-105' : 'opacity-60 hover:opacity-100'}`}
                             >
                        ...
                        <div className="grid grid-cols-2 gap-4 text-center">
                            <div className="p-4 bg-current bg-opacity-5 rounded-2xl">
            ...
            <motion.div initial={{y:"100%"}} animate={{y:0}} exit={{y:"100%"}} transition={{type:"spring", damping:25}} className={`fixed bottom-0 left-0 right-0 z-50 rounded-t-[2.5rem] p-8 pb-12 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] ${theme.cardBg} backdrop-blur-3xl`}>
                ...
                {isAmenAnimating ? (
                    ...
                    <form onSubmit={(e) => handleAmen(e, view === 'flow' ? "focus" : "manual")}>
                        {view === 'diary' && (
                            <input name="title" placeholder="Тема..." className="w-full bg-transparent border-b border-current border-opacity-10 py-3 text-lg font-light outline-none mb-4 placeholder:opacity-30" autoFocus />
                        )}
                        <textarea name="text" className="w-full bg-current bg-opacity-5 rounded-xl p-4 h-40 outline-none mb-6 text-base font-light placeholder:opacity-30 resize-none" placeholder={view === 'flow' ? "Твой отклик на слово..." : "Мысли, молитвы, благодарность..."} />
