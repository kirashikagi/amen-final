...
const CALENDAR_READINGS = {
  ...
  // ... (Остальные дни, сократил action для кнопок)
};
          ...
          <>
            ...
            >
              <div className="mt-20 flex flex-col items-start gap-6">
                {menuItems.map(item => (
                  <button key={item.id} onClick={() => { setView(item.id); setIsOpen(false); }} className={`text-left text-xl font-normal transition-opacity ${view === item.id ? 'opacity-100 font-medium' : 'opacity-50 hover:opacity-80'}`}>
                    {item.label}
                  </button>
                ))}
                
                <button onClick={() => { openThemeModal(); setIsOpen(false); }} className="text-left text-lg font-normal opacity-50 hover:opacity-100 flex items-center gap-3">
                    Атмосфера
                </button>

                {isAdmin && (
                    <button onClick={() => { setView('admin_feedback'); setIsOpen(false); }} className="text-left text-lg font-normal text-orange-600 hover:text-orange-700 flex items-center gap-3">
